...
#define DECORATE COM_DEBUG_PRINT("-------------\n")
...
int main(int32_t argc, char **argv){
    ...
    if(argc<4){
        DECORATE;
        COM_DEBUG_PRINT("Usage: rcp_tests s/g otherIP delay\n");
        COM_DEBUG_PRINT("s if this machine goes in space. g if it stays on ground\n");
        COM_DEBUG_PRINT("otherIP is the IP of the other machine\n");
        DECORATE;
        return -1;
    }

    uDelay = atoi(argv[3]);
    COM_DEBUG_PRINT("Delay set to %d MICROSECONDS\n", uDelay);

    groundMode = (strcmp(argv[1],"g")==0);


    if(groundMode){
        COM_DEBUG_PRINT("In ground mode.\n");
    } else{
        COM_DEBUG_PRINT("In space mode\n");
    }
    ...
    // DECORATE;
    // COM_DEBUG_PRINT("Testing open, bind, and close...\n");
    // simple_open_bind_close_test(otherIP);
    // COM_DEBUG_PRINT("Passed test\n");
    //
    #if DO_PACKET_STATS_TEST
    DECORATE;
    if(groundMode){
        COM_DEBUG_PRINT("Testing destination setup and send packets...\n");
        COM_DEBUG_PRINT("Let server test get to this point and hit enter.\n");
        simple_send_packets_test(otherIP);
        return 0;
    } else {
        COM_DEBUG_PRINT("Testing destination setup and receive packets...\n");
        simple_receive_packets_test(otherIP);
    }
    #endif
    //
    // DECORATE;
    // COM_DEBUG_PRINT("Testing connect and listen.\n");
    // if(groundMode){
    ...
    //
    // DECORATE;
    // COM_DEBUG_PRINT("Simple testing Send and Receive\n");
    ...
    DECORATE;
    COM_DEBUG_PRINT("Extensive testing Send and Receive\n");
    ...
    DECORATE;
    COM_DEBUG_PRINT("All RCP tests passed\n");
    return 0;
}
...
void simple_open_bind_close_test(char *otherIP){
    ...
    assert(fd>=0);
    COM_DEBUG_PRINT("Create socket passed\n");
    ...
    COM_DEBUG_PRINT("Bind passed\n");

    assert(rcp_close(&rcp_conn)==0);
    COM_DEBUG_PRINT("Close passed\n");
}
...
void simple_send_packets_test(char *otherIP){
    ...
    COM_DEBUG_PRINT("Attempting destination setup\n");
    assert(setupDest(&rcp_conn, otherIP, SPACE_PORT)==RCP_NO_ERROR);
    COM_DEBUG_PRINT("Setup destination passed\n");

    COM_DEBUG_PRINT("Attempting to send %d packets\n", SEND_PACKETS_COUNT_1);
    for(int32_t i=0; i<SEND_PACKETS_COUNT_1; i++){
        ...
        COM_DEBUG_PRINT("Sent packet %d of %ld bytes with dataSize %d bytes and content %s\n", i, PACKET_SERIAL_SIZE(packet), packet->dataSize, packet->data);
...
void simple_receive_packets_test(char *otherIP){
    ...
    int32_t count = 0;
    COM_DEBUG_PRINT("Attempting to receive packets\n");
    for(;;){
        Packet packet;
        struct timeval tv;
        ...
        if(recret<0){
            //COM_DEBUG_PRINT("Socket timed out.\n");
            continue;
        }
        count++;
        COM_DEBUG_PRINT("Received %d packets. This one with dataSize %d and content %s\n",count,packet.dataSize, packet.data);
        // COM_DEBUG_PRINT("Content Hex: \n");
        // for(int i=0;i<packet.dataSize;i++){
        //     COM_DEBUG_PRINT("0x%x ", packet.data[i]);
        // }
        // COM_DEBUG_PRINT("\n");
        destroyPacketData(&packet);
    }
    DECORATE;
    COM_DEBUG_PRINT("Received %d packets\n", count);
...
void simple_connect_test(char *otherIP){
    ...
    COM_DEBUG_PRINT("Connect finished\n");

    COM_DEBUG_PRINT("Attempting to close\n");
    assert(rcp_close(&rcp_conn)==0);
    COM_DEBUG_PRINT("Close passed\n");
}
...
void simple_listen_test(char *otherIP){
    ...
    COM_DEBUG_PRINT("Listen finished\n");
...
void simple_send_test(char *otherIP, uint8_t *simpleBuff, uint32_t len){
    ...
    COM_DEBUG_PRINT("Sending %d bytes\n", len);
    COM_DEBUG_PRINT("Attempting to send:\n");
    for(int i=0;i<len;i++){
        COM_DEBUG_PRINT("%c", simpleBuff[i]);
    }
    COM_DEBUG_PRINT("\n");
    rcp_send(&rcp_conn, simpleBuff, len);
    COM_DEBUG_PRINT("Send Finished\n");
...
void simple_receive_test(char *otherIP, uint8_t *simpleBuff, uint32_t len){
    ...
    uint32_t bytesRead;
    struct timeval to;
    COM_DEBUG_PRINT("Attempting to receive buffer of size %d.\n", len);
    rcp_receive(&rcp_conn, buf, len, &bytesRead, to);
    assert(bytesRead==len);
    COM_DEBUG_PRINT("Receive finished\n");
    // COM_DEBUG_PRINT("Should have received:\n");
    // for(int i=0;i<len;i++){
    //     COM_DEBUG_PRINT("0x%x ", simpleBuff[i]);
    // }
    // COM_DEBUG_PRINT("\n");
    // COM_DEBUG_PRINT("Received actually:\n");
    // for(int i=0;i<bytesRead;i++){
    //     COM_DEBUG_PRINT("0x%x ", buf[i]);
    // }
    // COM_DEBUG_PRINT("\nComparison:\n");
    // for(int i=0;i<bytesRead;i++){
    //     COM_DEBUG_PRINT("%d", buf[i]==simpleBuff[i]);
    // }
    // COM_DEBUG_PRINT("\n");
    assert(memcmp(buf, simpleBuff, len)==0);
    COM_DEBUG_PRINT("Received the correct data.\n");
    free(buf);
